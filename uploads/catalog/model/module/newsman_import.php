		...
		if (isset($data['last_data_time']) && $data['last_data_time'] < date("Y-m-d H:i:s", strtotime('-1 hour')))
		{
			if (isset($data['api_key'], $data['user_id'], $data['list_id'], $data['import_type']))
			{
				...
				if ($data['import_type'] == 1)
				{
					...
					for ($i = 0; $i < $rounds; $i++)
					{
						$query = $this->db->query("SELECT email FROM " . DB_PREFIX . "customer WHERE date_added > '" . $data['last_data_time'] . "' AND status = 1 AND approved = 1 AND newsletter = 1 ORDER BY customer_id ASC LIMIT " . ($i * 10000) . ", 10000");
						$csv = "email" . "," . "source" . PHP_EOL;
						foreach ($query->rows as $row)
							$csv .= $row['email'] . "," . "opencart newsman plugin subscriber" . PHP_EOL;
					...
					if ($data['import_type'] == 2 && isset($data['segments']))
					{
						...
						foreach ($segments as $cg => $seg)
						{
							if ($seg != 0)
							{
								if ($cg != "'0'")
								{
									$query = $this->db->query("SELECT COUNT(email) AS number FROM " . DB_PREFIX . "customer WHERE date_added > '" . $data['last_data_time'] . "' AND status = 1 AND approved = 1 AND newsletter = 1 AND customer_group_id = " . $cg);
									$rounds = ceil(intval($query->row['number']) / 10000);
									for ($i = 0; $i < $rounds; $i++)
									{
										$query = $this->db->query("SELECT email, firstname, lastname FROM " . DB_PREFIX . "customer WHERE date_added > '" . $data['last_data_time'] . "' AND status = 1 AND approved = 1 AND newsletter = 1 AND customer_group_id = " . $cg . " ORDER BY customer_id ASC LIMIT " . ($i * 10000) . ", 10000");
										$csv = "email,firstname,lastname,source" . PHP_EOL;
										foreach ($query->rows as $row)
											$csv .= $row['email'] . "," . $row['firstname'] . "," . $row['lastname'] . "," . "opencart newsman plugin subscriber" . PHP_EOL;
